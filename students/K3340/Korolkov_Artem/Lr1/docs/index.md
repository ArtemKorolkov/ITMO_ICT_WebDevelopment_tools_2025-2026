...
- Новая папка: `students/K3340/Korolkov_Artem/Lr2`
- Подзадачи:
  - `task1_sum` — сравнение `threading`, `multiprocessing`, `asyncio` на вычислении суммы
  - `task2_parsing` — параллельный парсинг сайтов и запись в БД ЛР1

### Задание 1. Сумма от 1 до 10_000_000_000_000

Реализованы три программы:

- `Lr2/task1_sum/threading_sum.py`
- `Lr2/task1_sum/multiprocessing_sum.py`
- `Lr2/task1_sum/async_sum.py`

Общее для всех вариантов:

- Диапазон делится на 8 равных подзадач.
- Используется функция `calculate_sum(start, end)`.
- Сумма диапазона считается по формуле арифметической прогрессии.
- Фиксируется время выполнения (`time.perf_counter()`).

Пример запуска:

```bash
cd students/K3340/Korolkov_Artem/Lr2/task1_sum
python threading_sum.py
python multiprocessing_sum.py
python async_sum.py
```

Таблица сравнения времени (замер на локальном запуске):

| Подход | Время, сек |
|---|-----------:|
| threading |   0.002580 |
| multiprocessing |   0.156592 |
| asyncio |   0.000170 |

Комментарий:

- Для чисто CPU-задачи `multiprocessing` обычно предпочтительнее `threading` из-за GIL.
- `asyncio` удобен в первую очередь для I/O операций, а не для тяжелых вычислений CPU.

### Задание 2. Параллельный парсинг страниц с сохранением в БД

Реализованы три программы:

- `Lr2/task2_parsing/threading_parser.py`
- `Lr2/task2_parsing/multiprocessing_parser.py`
- `Lr2/task2_parsing/async_parser.py`
...
- Загружает HTML.
- Парсит `<title>`.
- Сохраняет результат в таблицу `tag` из БД лабораторной 1.
- Выводит на экран обработанный URL и заголовок.
- Фиксирует общее время выполнения.

Пример запуска:

```bash
cd students/K3340/Korolkov_Artem/Lr2/task2_parsing
python threading_parser.py
python multiprocessing_parser.py
python async_parser.py
```

Таблица сравнения времени (заполните фактическими замерами):

| Подход |     Время, сек |
|---|---------------:|
| threading |            1.598 |
| multiprocessing |           2.031 |
| asyncio |          1.273 |

Комментарий:

- Для сетевого I/O обычно выигрывают `threading` и `asyncio`.
- `multiprocessing` может быть избыточным для I/O задач из-за накладных расходов на процессы.
- `asyncio` часто дает лучший баланс скорости и контролируемости при большом числе запросов.

### Итог по ЛР2

В ЛР2 реализованы все требуемые подходы (`threading`, `multiprocessing`, `asyncio`) для двух типов задач:

- CPU-bound: вычисление суммы большого диапазона.
- I/O-bound: массовый парсинг веб-страниц и запись результатов в БД.

Это показывает практические различия между потоками, процессами и асинхронной моделью в Python.
