...
def parse_and_save(url: str, conn) -> None:
    ...
    slug = f"lr2-mproc-{datetime.utcnow().strftime('%Y%m%d%H%M%S%f')}"
    ...
    conn.commit()
    print(f"[multiprocessing] saved: {url} -> {title}")
...
def detect_tag_column(conn) -> str:
    ...
    if fallback:
        chosen = fallback[0]
        print(f"[multiprocessing] using fallback column '{chosen}' in table 'tag'")
        return chosen
    with conn.cursor() as cursor:
        cursor.execute("ALTER TABLE tag ADD COLUMN name VARCHAR(255)")
    conn.commit()
    print("[multiprocessing] created missing column 'name' in table 'tag'")
    return "name"
    ...
    try:
        for url in urls:
            try:
                ...
                print(f"[multiprocessing] error for {url}: {exc}")
    finally:
        conn.close()


def main() -> None:
    groups = chunkify(URLS, PROCESSES)
    started_at = time.perf_counter()

    jobs = [mp.Process(target=worker, args=(group,)) for group in groups]
    for job in jobs:
        job.start()
    for job in jobs:
        job.join()
    ...
    print(f"[multiprocessing] elapsed: {elapsed:.3f} sec")
